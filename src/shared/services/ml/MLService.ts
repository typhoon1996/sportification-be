...
export class MLService {
  ...
  static async predictMatchOutcome(matchId: string) {
    try {
      ...
      const adjustedProb1 = Math.max(
        0.05,
        Math.min(0.95, expectedScore1 + formAdjustment)
      );
      ...
      return {
        ...
        factors: {
          ...
          experienceFactor: this.calculateExperienceFactor(
            player1Stats,
            player2Stats
          ),
        },
        recommendation:
          adjustedProb1 > 0.6
            ? "Strong favorite: Player 1"
            : adjustedProb2 > 0.6
              ? "Strong favorite: Player 2"
              : "Close match",
      };
    } catch (error) {
    ...
    try {
      ...
      const scoredMatches = await Promise.all(
        availableMatches.map(async match => {
          const score = await this.calculateMatchRecommendationScore(
            user,
            match,
            userActivity
          );
  ...
  static async analyzeChurnRisk(userId: string) {
    try {
      ...
      const [recentActivity, totalActivities, lastLogin, completedMatches] =
        await Promise.all([
      ...
      const daysSinceLastLogin = lastLogin
        ? Math.floor(
            (Date.now() - lastLogin.timestamp.getTime()) / (24 * 60 * 60 * 1000)
          )
        : 30;

      const activityTrend =
        recentActivity / Math.max(1, totalActivities - recentActivity);
  ...
  static async generateOptimalBracket(tournamentId: string) {
    try {
      const tournament =
        await Tournament.findById(tournamentId).populate("participants");
      ...
      participantStats.sort(
        (a, b) => (b.stats.rating || 1200) - (a.stats.rating || 1200)
      );
      ...
      for (const peakDay of peakDays) {
        for (const peakHour of peakHours) {
          ...
          recommendations.push({
            ...
            confidence: Math.min(
              0.95,
              (peakDay.count + peakHour.count) / Math.max(...dailyActivity) / 2
            ),
          });
        }
      }
  ...
  private static async getUserStats(userId: string) {
    ...
    const wins = matches.filter(
      match =>
        (match as any).result &&
        (match as any).result.winner?.toString() === userId
    ).length;
  ...
  private static calculateConfidence(stats1: any, stats2: any): number {
    ...
    const experienceFactor =
      Math.min(stats1.totalMatches, stats2.totalMatches) / 20;

    return Math.max(
      0.3,
      Math.min(0.95, 0.7 + experienceFactor - matchDifference * 0.01)
    );
  }
  ...
  private static async calculateMatchRecommendationScore(
    user: any,
    match: any,
    userActivity: any
  ): Promise<number> {
    ...
    const avgOpponentRating =
      opponentRatings.reduce((sum, stats) => sum + stats.rating, 0) /
