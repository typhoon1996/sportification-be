...
export const createRateLimiter = (
  ...
  uniquePrefix = 'default'
) => {
  const redisStore = getRateLimitStore(uniquePrefix);
  ...
  if (!redisStore && config.app.env !== 'test' && !rateLimitFallbackLogged) {
    logger.warn('Redis rate limit store unavailable; falling back to in-memory rate limiting');
