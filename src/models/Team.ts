...
import { ITeam, ITeamStatics, TeamRole } from '../types';
...
teamSchema.pre('save', function(next) {
  ...
  const captainIsMember = this.members.some(m => m.user.toString() === this.captain.toString());
  if (!captainIsMember) {
...
teamSchema.methods.addMember = function(userId: string, role: TeamRole = TeamRole.PLAYER) {
  if (this.isFull) {
...
teamSchema.methods.removeMember = function(userId: string) {
  ...
  this.members = this.members.filter((m: any) => m.user.toString() !== userId.toString());
...
teamSchema.methods.updateMemberRole = function(userId: string, role: TeamRole) {
...
teamSchema.methods.transferCaptaincy = function(newCaptainId: string) {
  ...
  const oldCaptain = this.members.find((m: any) => m.user.toString() === this.captain.toString());
  if (oldCaptain) {
  ...
  const newCaptain = this.members.find((m: any) => m.user.toString() === newCaptainId.toString());
  if (newCaptain) {
    newCaptain.role = TeamRole.CAPTAIN;
  }
};

export const Team = model<ITeam, Model<ITeam> & ITeamStatics>('Team', teamSchema);
